...
def require_role(role: str):
    async def role_dependency(payload: dict = Depends(get_current_user)):
        ...
        if role not in user_roles: # Ahora el rol es un string (no lista) en la creacion del token, pero verifiquemos.
            # En AuthService: "roles": user_data["rol"] -> "empleado" string.
            # Verificamos si es igual o esta contenido si fuera lista.
            if user_roles != role:
                 raise HTTPException(
